...
import { EMathTraits } from "./EMathTraits";

export const mathEquationList: (() => IMathEquation)[] = [
  () => new MathEquation("Product 1", "\\(f(a, b) = a \\cdot b\\)", (a: IMathVariable, b: IMathVariable) => a.v * b.v, [new MathVariable("a"), new MathVariable("b")], [EMathTraits.linear]),
  ...
  [new MathVariable("a", 20, 2), new MathVariable("b", 20), new MathVariable("c", 20)], [EMathTraits.approximation]),
  ...
  [new MathVariable("a", 20, 2), new MathVariable("b", 20), new MathVariable("c", 20)], [EMathTraits.approximation]),
  () => new MathEquation("Integral 1", "\\(f(a, b) = \\int_a^b 2 \\cdot \\sqrt a \\cdot x dx\\)", (a: IMathVariable, b: IMathVariable) => Math.max(Math.sqrt(a.v) * Math.pow(b.v, 2) - Math.pow(a.v, 2.5), 0), [new MathVariable("a"), new MathVariable("b")], [EMathTraits.integral]),
]
